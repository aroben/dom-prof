...
(function() {
  var addEventListenerCalls, computeByteSize, computeNodesStats, computeSerializedDomSize, countParentNodes, findGlobals, findInlineScripts, findInlineStyles, findJqueryEventHandlers, findScripts, findStylesheetLinks, hasInlineScript, inlineEventAttrs, jQuery, jqueryFindCalls, jqueryMatchCalls, jqueryReadyCalls, now, querySelectorAllCalls, querySelectorCalls, wrap;
  ...
  wrap(window, 'addEventListener', function(name, listener, capture) {
    return function() {
      return addEventListenerCalls.push({
        node: 'window',
        timestamp: now(),
        name: name,
        capture: capture
  ...
  wrap(document, 'querySelectorAll', function(selector) {
    return function(nodes) {
      return querySelectorAllCalls.push({
        timestamp: now(),
        selector: selector,
        nodes: nodes.length
      });
  ...
  wrap(document, 'querySelector', function(selector) {
    return function(node) {
      return querySelectorCalls.push({
        timestamp: now(),
        selector: selector,
        nodes: (node ? 1 : 0)
      });
